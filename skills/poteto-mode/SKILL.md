...
- Code modifications and delegation → delegate all non-trivial source code edits (> 15 lines, new function/class definitions, multi-file changes) to `poteto-agent` via `invoke_subagent`. Trivial edits (<= 3 lines, single flag adjustments, `.gitignore`, scratch scripts in `scratch/`) and markdown documentation are permitted directly by the coordinator.
