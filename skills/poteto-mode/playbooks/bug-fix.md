...
2. Binary-search the cause. Form the candidate hypotheses, then rule them out until one survives. Seed them with `how` over the affected subsystem and the **why** skill for regression history. Each pass, take the split that cuts the most remaining problem space, get runtime evidence, eliminate. When program state is unclear, add instrumentation or logging and read it as the code runs. Don't guess. Drive a long or stubborn hunt with Antigravity's `/goal` or `/schedule`. Confirm the surviving *mechanism* with runtime evidence before the step-3 architect/interrogate fan-out; a design grounded on a plausible-but-unconfirmed cause can be unanimously wrong while the real cause sits one subsystem over.
3. Plan the fix. If it crosses a function boundary, `architect` first. For complex fixes, capture the diagnosis and proposed changes in `<appDataDir>/brain/<conversation-id>/implementation_plan.md` artifact. Delegate implementation to a subagent using your configured bug-fix model (default `gemini-3.7-flash-high`) with a specific scope; review the diff.
4. Verify on the same surface; the original repro now passes. "Inconclusive" or wrong-surface is not a pass; flag it. Unit tests show branch behavior, not bug absence. Record the verbatim failing-then-passing repro output and validation evidence in `<appDataDir>/brain/<conversation-id>/walkthrough.md` artifact.
...
**Reply:** point to `walkthrough.md` and the PR. Summarize what was broken, root cause, fix, and verification outcome in concise declarative sentences. Detailed repro logs and traces live in the walkthrough artifact.
