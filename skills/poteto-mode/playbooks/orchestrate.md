...
Create `orchestrate/<project-slug>/` in the current agent's store (e.g. `<appDataDir>/brain/<conversation-id>/orchestrate/<project-slug>/` or `.agents/orchestrate/<project-slug>/`). Every file has exactly one writer; owners publish facts, readers aggregate at read time. Use `$(find ~/.gemini/config/plugins/agystack .agents/plugins/agystack skills/poteto-mode -name "orch" -type f 2>/dev/null | head -1)` (or `bun skills/poteto-mode/scripts/orch/orch.ts`) for bookkeeping, written below as `orch`, while its canonical plain TSV and JSON stay readable without the CLI.
...
- `inbox/` holds completion pointers. `gates.md` parks human gates (question, options, default on no answer) so a completion flood cannot wipe interactive state.
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (arm it via `/schedule` or `/goal`, with a long heartbeat fallback), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
