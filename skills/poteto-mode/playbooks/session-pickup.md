...
1. Locate the prior trail. Check the artifacts directory `<appDataDir>/brain/<conversation-id>/` for existing `implementation_plan.md`, `walkthrough.md`, `resume_checkpoint.md`, or `decision_trail.md`. Read the local transcript under `~/.gemini/antigravity-ide/brain/<conversation-id>/.system_generated/logs/transcript.jsonl` (or `transcript_full.jsonl`; the system prompt names the path), a cloud-agent URL, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
