...
1. Locate the prior trail. Check the artifacts directory `<appDataDir>/brain/<conversation-id>/` for existing `implementation_plan.md`, `walkthrough.md`, `resume_checkpoint.md`, or `decision_trail.md`. Read the local transcript under `<appDataDir>/brain/<conversation-id>/.system_generated/logs/transcript.jsonl` (or `~/.gemini/antigravity/brain/` / `~/.gemini/antigravity-ide/brain/`, or `transcript_full.jsonl`; the system prompt names the path), a cloud-agent URL, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
...
**Reply:** where the prior agent stopped, what you inherited vs redid (ideally nothing redone), the resume point, and the outcome in concise declarative sentences. Link to any prior artifacts read.
