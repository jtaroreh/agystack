...
5. Verify on the matching surface. For a visual decision, screenshot each variant via the control skill and drive the interaction; the eye is the test. Copy screenshots to `<appDataDir>/brain/<conversation-id>/` and embed them in a `prototype_review.md` artifact using `carousel` blocks or side-by-side comparisons. For a behavioral or timing decision, observe the thing you are deciding by logging the timing, printing the output, or watching the render. The observation is the test here, not an assertion.
6. Present alternatives, tradeoffs, and a recommendation in `<appDataDir>/brain/<conversation-id>/prototype_review.md` artifact. The output is the decision plus the throwaway artifact, not shippable code. Hand the chosen direction to **Feature** (or `architect` for the shape) for the real build.

**Reply:** point to `prototype_review.md`. Summarize the variants explored, key observation, recommendation, and scratch path in concise declarative sentences. Visual carousels and detailed trade-offs live in the artifact. Say plainly that the prototype is throwaway.
