...
6. Hand back a cited diagnosis, no fix unless asked. Publish the complete trace breakdown, query tables, and source mappings to `<appDataDir>/brain/<conversation-id>/forensics_report.md` artifact. Route to Bug fix or Perf issue once the cause is known. Throughput checkpoint stays one line: `throughput checkpoint: n/a, read-only forensics`.

**Reply:** point to `forensics_report.md`. Summarize the artifact format, reduced finding, source location, and whether a paired capture confirmed it in concise declarative sentences. Detailed call trees and query results live in the artifact.
