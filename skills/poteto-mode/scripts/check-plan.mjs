...
const LANES_PATTERN = /Ten lanes on `(?:flash|gemini-3\.7-flash-fast)` at the PR head/;
const SUB_BLOCKS = [
	"Depends on.",
	"Files.",
	"Build.",
...
for (const pr of prSections) {
	...
	if (live) {
		if (!LANES_PATTERN.test(live.rest)) fail(live.n, `${pr.title}: Verify, live lacks "Ten lanes on \\\`flash\\\` at the PR head"`);
