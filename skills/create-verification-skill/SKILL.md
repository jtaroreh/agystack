...
Every serious project needs a scripted way to drive the real app and prove behavior: launch it, exercise a feature the way a user would, and capture evidence. Antigravity already provides native browser automation (`browser_subagent` via Chrome DevTools MCP) and process execution. There is no need to write or depend on generic browser driver packages. The high-leverage focus of this skill is entirely on discovering and generating the project-local skill (`.agents/skills/verify-<app>/`) and its application **feature map** (`features/`) with stable selectors.
...
- **Drive:** how can an agent interact with it programmatically? Existing harnesses first: Playwright/Cypress specs, expect scripts, PTY helpers, curl-able endpoints, a debug port. For web and Electron, use native `browser_subagent` / CDP with repo-specific routes and selectors. For CLI/TUI, use a tmux/PTY harness. For services, use plain HTTP.
