...
- Tool calls against any `SKILL.md` file (workspace `.agents/skills/`, user-level `~/.gemini/config/skills/`, or plugin-installed paths under `~/.gemini/config/plugins/`)
- Subagent prompts that name a skill path
- Tool calls (run_command, view_file, grep_search, MCP, etc.) that match a skill's documented commands
