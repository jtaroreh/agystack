...
This skill orchestrates three others: an inline mining pass (see step 1), Antigravity skill authoring (a `SKILL.md` under `.agents/skills/` or `~/.gemini/config/skills/`), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `.agents/skills/**/*-mode/SKILL.md` and `~/.gemini/config/skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`.agents/skills/<handle>/`), not only at the top level. If one exists, confirm intent with numbered options (unless they already said "update my skill" or similar):
...
Mining misses intent that hasn't come up yet. Ask with numbered multi-choice options rather than asking the user to type from scratch. Lower cognitive load, higher hit rate.
...
Author the skill as a `SKILL.md` with YAML frontmatter (`name`, `description`). Placement:
...
- Antigravity skill authoring: `name` and `description` frontmatter in `SKILL.md`.
