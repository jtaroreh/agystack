...
Transcripts live under `~/.gemini/antigravity-ide/brain/<conversation-id>/` (Antigravity IDE) and `~/.gemini/antigravity/brain/<conversation-id>/` (Antigravity 2.0). Prefer `.system_generated/logs/transcript.jsonl` or `transcript_full.jsonl`. The current session's `transcriptPath` (from hooks or system context) is authoritative. Do not glob `~/.cursor/projects/*/`.
...
3. Fan out across your chat history. Spawn parallel subagents on a fast, cheap model, each taking a slice of the corpus, since searching transcripts is grunt work. Tell every subagent to order candidates by real modification time (`ls -t`) and never by UUID name, grep the topic first and then read only the matching chats and only their relevant regions, and skip the current chat plus obvious noise (subagent, eval, and test chats). Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the chat with dual-identifier links: `[<Topic or Brief>](conversation://<id>) (<id>)`. For one or two chats, skip the fan-out and search directly. The raw transcripts stay in the subagents. The main thread gets only their findings.
...
An adjacent feature or ticket stays out unless it blocks this one. When the capsule and thread lines outgrow a screen, cut detail before you cut threads. Write the brief through the **unslop** skill, cite chat findings using `[<Topic or Brief>](conversation://<id>) (<id>)` and shared-record findings by their source (PR #, ticket ID, chat permalink, error-tracker issue), and sanitize private context before any public output.
