...
1. **Index and helper hygiene.** Read the feature map README and glob its sibling files in `features/`. Fix missing, extra, duplicate, or dead entries. Check all scripts in `scripts/` against the skill's `## Helpers` and `## Drive` sections: detect unregistered helper scripts, missing executable permissions, or orphaned utilities. Audit helper CLI contracts and exit codes: ensure status-checking or polling helpers do not emit fatal non-zero exit codes for in-flight domain states (e.g. pending validation), and verify default timeouts accommodate realistic grader queues. Lightweight; no generated inventory.
