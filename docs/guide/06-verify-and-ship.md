...
When verifying or investigating, enforce strict tool precedence:

1. **Filesystem Tools (`view_file`, `grep_search`, `find_by_name`)**: Ground truth for code, configs, AST, tests, and repo docs. Never use a browser or UI tools to view, search, or scroll through code and repo files.
2. **CLI / Runtime Tools (`run_command`)**: Test suites, API endpoints, build/server logs, process lifecycles, and exit codes.
3. **Browser Tools (`browser_subagent` / Chrome DevTools MCP)**: Strictly for live UI interaction, DOM layout inspection, CSS styles, user events, and rendered visual screenshots.

## Create a project verification skill

The UI bullet above hides a real requirement: the agent needs a scripted way to drive your app. Antigravity already includes native browser automation as `browser_subagent` (Chrome DevTools MCP), so there is no need to wait on generic driver packages (like cursor-team-kit or control-ui).

The high-leverage asset needed for any project is the app's **feature map** (generated by `/create-verification-skill`). If your project lacks one, run:

```text
/create-verification-skill
```

[`/create-verification-skill`](../../skills/create-verification-skill/SKILL.md) interviews the repository, not you. It works out what a user touches, how the app launches locally, what drives it (an existing harness first, otherwise native `browser_subagent` / CDP, a PTY, or plain HTTP), what evidence proves behavior, and whether two instances can run side by side. It asks you only what the code can't answer.
